...
def format_fastq_headers(in_fastq_1, in_fastq_2, outdir):
    out_fastq_1 = os.path.join(outdir,
                               '{0}.headersRenamed_1.fq'.format(os.path.splitext(os.path.basename(in_fastq_1))[0]))
    out_fastq_2 = os.path.join(outdir,
                               '{0}.headersRenamed_2.fq'.format(os.path.splitext(os.path.basename(in_fastq_2))[0]))
...
def compression_type(file_to_test):
...
def main():
    parser = argparse.ArgumentParser(prog='renamePE_samtoolsFASTQ.py',
                                     description='Rename the fastq headers with PE terminations that were not include '
                                                 'in samtools fastq command',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--version', help='Version information', action='version', version=str('%(prog)s v' + version))

    parser_required = parser.add_argument_group('Required options')
    parser_required.add_argument('-1', '--fastq_1', type=argparse.FileType('r'), metavar='/path/to/input/file_1.fq',
                                 help='Uncompressed fastq file containing mate 1 reads', required=True)
    parser_required.add_argument('-2', '--fastq_2', type=argparse.FileType('r'), metavar='/path/to/input/file_2.fq',
                                 help='Uncompressed fastq file containing mate 2 reads', required=True)

    parser_optional_general = parser.add_argument_group('General facultative options')
    parser_optional_general.add_argument('-o', '--outdir', type=str, metavar='/output/directory/',
                                         help='Path for output directory', required=False, default='.')
    ...
    for fastq in fastq_files:
        if compression_type(fastq) is not None:
    ...
    number_reads, outfiles = format_fastq_headers(fastq_files[0], fastq_files[1], outdir)
